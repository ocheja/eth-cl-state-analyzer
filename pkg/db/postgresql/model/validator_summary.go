	...
	ON CONFLICT ON CONSTRAINT t_validator_summary_pkey
		DO 
			UPDATE SET 
				...
				f_start_epoch = excluded.f_start_epoch, 
				f_end_epoch = excluded.f_end_epoch, 
				...
				f_acc_base_reward = excluded.f_acc_base_reward,
...
type ValidatorSummary struct {
	...
	StartEpoch       uint64 // first epoch we measured this validator
	EndEpoch         uint64 // last epoch we measured this validator
	CurrentBalance   int64  // balance at the last measured epoch
	AccReward        int64  // Cumulative Reward
	AccMaxReward     int64  // Cumulative MaxReward
	AccBaseReward    int64  // Cumulative BaseReward
